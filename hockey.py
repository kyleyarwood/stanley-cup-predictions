...
def get_nba_team_data_for_years_from_file(start_year, end_year):
    ...
    for team in teams:
        ...
        for season in seasons:
            ...
            stats_columns["team"].append(
                nba_acronyms[name] if name in nba_acronyms else None)
            ...
            if stats_columns["PO_LOSSES"][-1] == 0:
                ...
                stats_columns["playoffRoundWins"].append(
                    stats_columns["PO_WINS"][-1]//4)
    df = pandas.DataFrame.from_dict(stats_columns)
    print(df)
    return df
...
def get_nba_team_data_for_years(start_year, end_year):
    ...
    for tm in tms:
        print(tm)
        try:
            x = teamyearbyyearstats.TeamYearByYearStats(
                tm['id']).nba_response._response
            with open('nbaresponses.txt', 'r') as f:
                x = f.read
            stats = loads(x)
        except:
...
def normalize(df, skip_cols=[]):
    # print(df.columns)
    for col in df.columns:
        print(col)
        if col in skip_cols or col in ('team', 'season'):
            continue
        df[col] = df[col].apply(lambda x: float(x))
        mx, mn = max(df[col]), min(df[col])
        if mx-mn == 0:
            continue
        df[col] = (df[col] - mn)/(mx - mn)


def create_db(start_year, end_year, conn, cur):
    df_created = False
    for i in range(end_year, start_year - 1, -1):
        season = str(i) + str(i + 1)
        new_df = get_season_data(info, season, options)
        normalize(new_df, skip_cols=['playoffRoundWins'])
        if df_created:
...
def get_hockey_data_from_db(conn):
...
def create_model_for_year(df, yr_to_care_about):
    model = Ridge()
    df = df[df['season'] != yr_to_care_about]
...
def main():
    ...
    db_exists = does_db_exist(conn=conn, cur=cur)

    if not db_exists:
        create_db(start_year=2005, end_year=2021, conn=conn, cur=cur)

    df = get_hockey_data_from_db(conn=conn)

    yr_to_care_about = '20212022'
    #_2021_df = get_season_data(info, '20202021', options)
    _2020_df = get_season_data(info, yr_to_care_about, options)
    normalize(_2020_df, skip_cols=['playoffRoundWins'])
    teams_to_care_about = {'STL', 'COL', 'CGY',
                           'EDM', 'NYR', 'CAR', 'FLA', 'TBL'}
    teams_in_order = _2020_df[_2020_df['team'].isin(teams_to_care_about)]
    indices = set(teams_in_order.index)
    teams_in_order = teams_in_order['team'].tolist()
    print(indices)
    # print(_2021_df)

    model = create_model_for_year(df=df, yr_to_care_about=yr_to_care_about)
    ...
    team_preds = [e for i, e in enumerate(
        _2020_pred_values) if i in indices]
    zipped = list(map(str, zip(team_preds, teams_in_order)))
    print('\n'.join(sorted(zipped)))
...
def nba_main():
    ...
    teams_to_care_about = {'STL', 'COL', 'CGY',
                           'EDM', 'NYR', 'CAR', 'FLA', 'TBL'}
    print(_2020_df[_2020_df['team'].in_(teams_to_care_about)])
    _2020_wins = _2020_df['PO_WINS']
    _2020_df.drop(['team', 'TEAM_ID', 'TEAM_CITY', 'TEAM_NAME', 'YEAR', 'NBA_FINALS_APPEARANCE',
                  'PO_WINS', 'PO_LOSSES', 'playoffRoundWins'], axis=1, inplace=True)
    df = df[df['YEAR'] != '2019-20']
    model = Ridge()
    useful_stats = df.drop(['team', 'TEAM_ID', 'TEAM_CITY', 'TEAM_NAME', 'YEAR',
                           'NBA_FINALS_APPEARANCE', 'playoffRoundWins', 'PO_LOSSES'], axis=1)
